...
test('renders Loading component when prop show is null', () => {
    ...
    const load = screen.queryByText(/fetching data/i);
...
test('component renders when no seasons are selected and when rerenders with a season passed in', () => {
    ...
    let episodeSelector = screen.queryByTestId(/episodes-container/i);
    expect(episodeSelector).not.toBeInTheDocument();

    rerender(<Show show={testShow} selectedSeason={1}/>)
    episodeSelector = screen.queryByTestId(/episodes-container/i);
